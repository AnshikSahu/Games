...
class Piece:
    ...
    def add_if_legal(self, moves, move, check, pin):
        if move is None:
            return False
        if move.row in [3,4,5] and move.column in [3,4,5]:
            return False
...
class Pawn(Piece):
    ...
    def possible_moves(self, check):
        ...
        moves = []
        r=self.square.row
        c=self.square.column
        list={'front':self.board.get_square(r+1,c),'left':self.board.get_square(r,c-1),\
              'right':self.board.get_square(r,c+1),'back':self.board.get_square(r-1,c),\
              'right_up_diagonal':self.board.get_square(r+1,c+1),'right_down_diagonal':self.board.get_square(r-1,c+1),\
              'left_up_diagonal':self.board.get_square(r+1,c-1),'left_down_diagonal':self.board.get_square(r-1,c-1)}
        if(r in [1,2] and c in [3,4,5]):
            list['back']=None
            list['right']=None
            list['right_down_diagonal']=None
            list['right_up_diagonal']=None
        elif(r in [6,7] and c in [3,4,5]):
            list['left']=None
            list['left_down_diagonal']=None
            list['left_up_diagonal']=None
            list['front']=None
        elif(r in [3,4,5] and c in [1,2]):
            list['back']=None
            list['left_down_diagonal']=None
            list['right_down_diagonal']=None
            list['left']=None
        elif(r in [3,4,5] and c in [6,7]):
            list['front']=None
            list['left_up_diagonal']=None
            list['right_up_diagonal']=None
            list['right']=None
        elif(r in [1,2] and c in [1,2]):
            list['back']=None
            list['right']=None
            list['right_down_diagonal']=None
            list['left_down_diagonal']=None
        elif(r in [1,2] and c in [6,7]):
            list['front']=None
            list['right_down_diagonal']=None
            list['right_up_diagonal']=None
            list['right']=None
        elif(r in [6,7] and c in [1,2]):
            list['back']=None
            list['left_down_diagonal']=None
            list['left_up_diagonal']=None
            list['left']=None
        elif(r in [6,7] and c in [6,7]):
            list['left']=None
            list['right_up_diagonal']=None
            list['left_up_diagonal']=None
            list['front']=None
            
        for squ in list.values():
            if squ is not None:
                if check is None or not check.restricted(squ):
                    if pin is None or not pin.restricted(squ):
                        moves.append(squ)
        return moves
...
class King(Piece):
    ...
    def move(self, square):
        if self.color == 'White':
            ...
            if self.square.row == 7 and self.square.column == 5:
                if square.row == 7:
                    # if square.column == 7:
                    #     rook = self.board.get_square(8, 8).piece
                    #     rook.move(self.board.get_square(8, 6))
                    if square.column == 3:
                        rook = self.board.get_square(7, 1).piece
                        rook.move(self.board.get_square(7, 4))
    ...
    def castle_available(self, moves):
        ...
        if self.color == 'White':
            short_rook = self.board.get_square(1, 7).piece
            middle_square1 = self.board.get_square(1, 5)
            middle_square2 = self.board.get_square(1, 6)
            if isinstance(short_rook, Rook) and not short_rook.moved:
                if middle_square1.piece is None and middle_square2.piece is None:
                    if self.in_check(middle_square1) is None and self.in_check(middle_square2) is None:
                        moves.append(self.board.get_square(1, 6))
            ...
            short_rook = self.board.get_square(7, 7).piece
            middle_square1 = self.board.get_square(7, 5)
            middle_square2 = self.board.get_square(7, 6)
            if isinstance(short_rook, Rook) and not short_rook.moved:
                if middle_square1.piece is None and middle_square2.piece is None:
                    if self.in_check(middle_square1) is None and self.in_check(middle_square2) is None:
                        moves.append(self.board.get_square(8, 7))

            long_rook = self.board.get_square(7, 1).piece
            middle_square1 = self.board.get_square(7, 2)
            middle_square2 = self.board.get_square(7, 3)
            middle_square3 = self.board.get_square(7, 4)
            if isinstance(long_rook, Rook) and not long_rook.moved:
                if middle_square1.piece is None and middle_square2.piece is None:
                    if middle_square3.piece is None:
                        if self.in_check(middle_square2) is None and self.in_check(middle_square3) is None:
                            moves.append(self.board.get_square(7, 3))
